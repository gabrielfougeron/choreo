...
def Integrate(n_NT_init):
    ...
    while(GoOn):
        ...
        GoOn = (i_try < i_try_max) and  (period_err > period_err_wish) and not(StopNext)

        StopNext = (period_err < period_err_max)
    ...
    if (period_err < period_err_max):
        ...
        print(f'Numerical Tank {n_NT_init:4d} could not integrate. Error: {period_err}')
