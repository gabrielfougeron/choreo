...
def Integrate(n_NT_init):
    ...
    while(GoOn):
        ...
        # Initial positions: (-2 c1, 0), (c1, c2), (c1, -c2) * 3
        # Initial velocities: (0, -2 c4), (c3, c4), (-c3, c4)


        x0[0] = -2 * all_NT_init[n_NT_init,0] *3
        x0[1] = 0
        x0[2] =   all_NT_init[n_NT_init,0]*3
        x0[3] =   all_NT_init[n_NT_init,1]*3
        x0[4] =   all_NT_init[n_NT_init,0]*3
        x0[5] = - all_NT_init[n_NT_init,1]*3
...
# 
# 
#         xf = np.ascontiguousarray(all_pos[nthird-1,:].copy())
#         vf = np.ascontiguousarray(all_v[nthird-1,:].copy())
...
# 
#             for ishift in range(nbody-1):
# 
#                 nbeg = (ishift+1) * nthird
#                 nend = (ishift+2) * nthird
# 
#                 all_pos[ib,:,nbeg:nend] = all_pos[(ib+nbody+2-ishift)%nbody,:,0:nthird]
