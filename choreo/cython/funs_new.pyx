...
cpdef void real_of_matmul(
    double complex[:,::1] a ,
    double complex[:,::1] b ,
    double[:,::1] c         ,
) noexcept nogil:

    cdef int ninter = c.shape[0]
    cdef int geodim = c.shape[1]

    cdef int ncom =  a.shape[0]

    cdef double* ifft_b_real = <double*> &a[0,0]
    cdef double* params_basis_reoganized_real = <double*> &b[0,0]
