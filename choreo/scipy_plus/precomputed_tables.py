...
from choreo.scipy_plus.cython.ODE import ExplicitRKTable
...
SymplecticEuler = ExplicitRKTable(
    c_table = np.array([1.])    ,
    d_table = np.array([1.])    ,
    th_cvg_rate = 1             ,
)

StormerVerlet = ExplicitRKTable(
    c_table = np.array([0.    ,1.      ])   ,
    d_table = np.array([1./2  ,1./2    ])   ,
    th_cvg_rate = 2                         ,
)

Ruth3 = ExplicitRKTable(
