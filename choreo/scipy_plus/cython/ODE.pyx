...
cdef class ImplicitSymplecticRKTable:
    ...
    def symmetric_adjoint(self):
        ...
        b_table_sym = np.empty((n), dtype=np.float64)
        c_table_sym = np.empty((n), dtype=np.float64)
        beta_table_sym = np.empty((n,n), dtype=np.float64)
        gamma_table_sym = np.empty((n,n), dtype=np.float64)
