...
def matmulNN(aN, bN, cN, aT, bT, cT):
    np.matmul(aN, bN, out=cN)
# def matmulNN(aN, bN, cN, aT, bT, cT):
#     np.matmul(aN, bN, out=cN)
# def matmulNN(aN, bN, cN, aT, bT, cT):
#     np.matmul(aN, bN, out=cN)
def matmulTT(aN, bN, cN, aT, bT, cT):
    np.matmul(bT, aT, out=cT)
...
def setup_abc(P, R, Q, real_dtype):
    aN = np.random.random((P,R)).astype(dtype=dtypes_dict[real_dtype])
    bN = np.random.random((R,Q)).astype(dtype=dtypes_dict[real_dtype])
    cN = np.zeros((P,Q),dtype=dtypes_dict[real_dtype])
    aT = aN.T
    bT = bN.T
    cT = np.ascontiguousarray(cN.T)
    return {'aN':aN, 'bN':bN, 'cN':cN,'aT':aT, 'bT':bT, 'cT':cT}
...
# # %%
# 
basename = 'matmul_timings_Transpose'
...
relative_to_val = {
    "real_dtype": 'float64'  ,
    pyquickbench.fun_ax_name :  'matmulNN'   ,
}
