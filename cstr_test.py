...
input_filename = './Sniff_all/1x1x1x1x1/1'
...
nbody = np.ones((nloop),dtype=int)
mass = np.ones((nloop))

change_var_coeff = 1.
# ~ change_var_coeff = 0.

coeff_to_param , param_to_coeff = setup_changevar(nloop,nbody,ncoeff,mass,MomCons=True,n_grad_change=change_var_coeff)
# ~ coeff_to_param , param_to_coeff = setup_changevar(nloop,nbody,ncoeff,mass,MomCons=False,n_grad_change=change_var_coeff)
    
print('Number of coeffs : ',coeff_to_param.shape[1])
print('Number of params : ',coeff_to_param.shape[0])
print('Number of detected independant constraints : ',coeff_to_param.shape[1]-coeff_to_param.shape[0])
print('NNZ in change var : ',coeff_to_param.getnnz())

    
x0,callfun = Package_args(nloop,nbody,ncoeff,mass,nint,all_coeffs,coeff_to_param,param_to_coeff)
